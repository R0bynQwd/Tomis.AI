...
print("STRESS TEST: 1000x matrix mult (10000x10000)")
try:
    if gpu:
...
def test_docker_runtime():
    ...
    cmd = "docker info 2>/dev/null | grep -i 'runtime' || echo 'Docker unavailable'"
    output = run_kubectl_exec(cmd)
    for line in output.split('\n'):
        if line.strip():
            print(f"  {line}")
...
if __name__ == "__main__":
    ...
    if "gpu-test" not in result.stdout:
        print("[ERROR] gpu-test pod not found. Create it first:")
        print("   kubectl apply -f gpu-test-pod.yaml")
        sys.exit(1)
    print("[OK] gpu-test pod ready\n")
